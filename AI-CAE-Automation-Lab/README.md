...
The MCP server currently exposes environment checks, skill listings, adapter listings, run record creation, evidence scanning, report generation, and minimal solver adapters for Abaqus, Fluent, and PCSCHEMATIC checks. Solver adapters write logs and return status objects; they do not claim engineering success without evidence.
