...
2. Ask the toolbox for a bridge plan when a solver adapter is involved:

```powershell
ai-cae-toolbox bridge-plan --solver <solver> --objective "<objective>"
```

3. Create a run directory before generating solver files:
...
4. Generate a tiny smoke template when available:

```powershell
ai-cae-toolbox write-smoke-template --solver <solver> --run-dir <run-dir> --case <case-name>
```

5. Put source artifacts in the run folder:
...
6. After execution, scan evidence:
