...
from .fluent import resolve_fluent_command
...
def resolve_workbench_command(workbench_command: str | None = None, ansys_root: str | None = None) -> str | None:
    configured = configured_solver_paths(load_toolbox_config(), "ansys-workbench")
    ansys_config = configured_solver_paths(load_toolbox_config(), "ansys")
    root = ansys_root or os.environ.get("ANSYS_ROOT") or configured.get("root") or ansys_config.get("root")
    version_roots = _ansys_root_candidates(root)
    candidates: list[str | Path | None] = [
        workbench_command,
        os.environ.get("WORKBENCH_EXE"),
        os.environ.get("ANSYS_WORKBENCH_EXE"),
        configured.get("workbench_exe"),
        ansys_config.get("workbench_exe"),
...
def ansys_check_install(
    ...
    configured = configured_solver_paths(load_toolbox_config(), "ansys")
    workbench_config = configured_solver_paths(load_toolbox_config(), "ansys-workbench")
    root = (
        ansys_root
        or os.environ.get("ANSYS_ROOT")
        or os.environ.get("FLUENT_ROOT")
        or configured.get("root")
        or workbench_config.get("root")
    )
    fluent = resolve_fluent_command(fluent_exe)
    workbench = resolve_workbench_command(workbench_exe, ansys_root=root)
    ...
    runnable = checks["fluent_exe"]["exists"] or checks["workbench_exe"]["exists"]
    has_root = checks["ansys_root"]["exists"]
    return {
        "status": "ok" if runnable else ("partial" if has_root else "missing"),
        "checks": checks,
        "message": "This check requires a real Fluent or Workbench executable; an Ansys directory alone is not enough.",
    }
